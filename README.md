...
Text summaries use OMP's dedicated generic `tinyModelClient` completion client, while title generation remains behind the title-policy `tinyTitleClient` facade. Summarization requires an OMP source checkout containing the unreleased generic-client split; the currently published dependency does not contain `@oh-my-pi/pi-coding-agent/tiny/model-client`. The optional SDK is loaded only when a summary is requested, so other annotation features still load on published OMP versions; an unavailable SDK triggers the verbatim-source fallback and warning. No released minimum version is claimed here—run with a source checkout that includes this change (or a later release once available) to enable small-model summaries.
