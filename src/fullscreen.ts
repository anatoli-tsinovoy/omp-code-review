...
import type { TextReviewOverlayResult, TextReviewSource } from "./text-types";
...
/** Mounts a frozen plain-text source in the same annotation overlay UX. */
export async function showTextReviewOverlay(
  ctx: ExtensionCommandContext,
  ...
  return ctx.ui.custom<TextReviewOverlayResult | undefined>(
    (tui, theme, keybindings, done) => {
      ...
      const complete = (result: TextReviewOverlayResult | undefined): void => {
        if (completed) return;
        completed = true;
        disposeOverlay();
        done(result);
      ...
      const overlay = new CodeReviewOverlay(tui, theme, keybindings, source, {
        cwd: ctx.cwd,
        onComplete: complete,
        onWarning: (message) => ctx.ui.notify(message, "warning"),
      });
