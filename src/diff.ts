...
function parseHeaderPaths(header: string): {
  oldPath?: string;
  newPath?: string;
} {
  ...
  const newToken =
    oldToken === undefined ? undefined : readGitToken(payload, oldToken.next);
  return {
    oldPath:
      oldToken === undefined
        ? undefined
        : stripDiffPrefix(decodeGitPath(oldToken.token)),
    newPath:
      newToken === undefined
        ? undefined
        : stripDiffPrefix(decodeGitPath(newToken.token)),
  };
}
...
function splitFileDiffs(rawDiff: string): string[] {
  const starts: number[] = [];
  for (const match of rawDiff.matchAll(/^diff --git /gm))
    starts.push(match.index!);
  return starts.map((start, index) =>
    rawDiff.slice(start, starts[index + 1] ?? rawDiff.length).trimEnd(),
  );
}
...
export function parseReviewDiffSnapshot(rawDiff: string): ReviewDiffSnapshot {
  ...
  for (const rawFileDiff of splitFileDiffs(rawDiff)) {
    ...
    const exclusionReason = EXCLUDED_PATTERNS.find((entry) =>
      entry.pattern.test(path),
    )?.reason;
    if (exclusionReason !== undefined) {
      excluded.push({ path, reason: exclusionReason });
      continue;
    }
...
export function getRecommendedReviewAgentCount(
...
export function getReviewDiffPreview(
  rawDiff: string,
  maxLines: number,
): string {
